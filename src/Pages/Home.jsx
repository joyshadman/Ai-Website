...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#030303] text-white overflow-x-hidden selection:bg-purple-500/30 font-sans">
      ...
      {/* --- BACKGROUND DECOR (Animated Grid and Glows) --- */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none -z-10">
        
        {/* Pulsing Static Glows */}
        ...
        <motion.div 
          animate={{ 
            x: [0, -40, 0], // Subtle horizontal drift
            y: [0, -40, 0]  // Subtle vertical drift
          }}
          transition={{
      ...
      <section className="relative pt-44 pb-20 px-6">
        <div className="max-w-7xl mx-auto text-center">
          ...
          {/* --- AI CHAT COMMAND CENTER (Bigger & More Animated) --- */}
          <motion.div
          ...
          >
            {/* Pulsing Outer Glow around input */}
            <motion.div 
              animate={{ opacity: [0.2, 0.5, 0.2], scale: [1, 1.02, 1] }}
              transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
              className="absolute -inset-2 bg-gradient-to-r from-purple-600/30 to-blue-600/30 rounded-[2.5rem] blur opacity-20 transition duration-1000 group-hover:opacity-60 group-hover:duration-200"
            />
            ...
            <GlassCard className="relative p-3 flex flex-col md:flex-row items-center gap-3 border-white/20 shadow-[0_0_60px_-10px_rgba(168,85,247,0.15)] overflow-hidden">
              ...
              {/* -- INNER NEURAL FLOW ANIMATION -- */}
              {/* This is a subtle gradient layer inside the input field itself */}
              <motion.div
                animate={{
                  background: [
              ...
              <div className="flex-1 flex items-center px-6 w-full relative z-10">
                <BotMessageSquare className="w-8 h-8 text-purple-400 mr-5 shrink-0" />
                <input 
                  type="text"
                  value={inputValue}
                  onChange={(e) => setInputValue(e.target.value)}
                  placeholder="Describe your site (e.g. 'Build a glassy SaaS dashboard for an analytics startup')"
                  className="w-full bg-transparent py-7 text-2xl outline-none placeholder:text-gray-600 text-white font-medium"
                />
              </div>

              {/* Generate Button (Bigger and glowing) */}
              <button className="w-full md:w-auto bg-white text-black p-6 rounded-2xl font-black text-lg flex items-center justify-center gap-3 hover:bg-purple-500 hover:text-white transition-all shadow-xl active:scale-95 group relative z-10 shrink-0">
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3 relative z-10">
              {['Dark Portfolio', 'Web3 Platform', 'E-commerce UI', 'Design Studio'].map((tag) => (
                <button 
                  key={tag}
                  className="px-5 py-2.5 rounded-full border border-white/5 bg-white/5 text-sm font-medium text-gray-400 hover:border-purple-500/50 hover:text-purple-400 transition-all hover:bg-white/[0.08]"
                >
                  {tag}
                </button>
              ))}
        ...
        {/* Dynamic Preview Section (Secondary Focus) */}
                ...
                <div className="relative aspect-[16/10] bg-[#080808] rounded-2xl border border-white/5 overflow-hidden">
                  ...
                  <div className="pt-24 px-12">
                    <motion.div 
                      ...
                      className="w-2/3 h-10 bg-gradient-to-r from-purple-500/20 via-blue-500/10 to-transparent rounded-lg mb-6" 
                    />
                  ...
                  {/* Floating Action Indicator (Animated Pointer) */}
                  <motion.div 
