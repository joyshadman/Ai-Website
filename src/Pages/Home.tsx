...
const GlassCard: React.FC<GlassCardProps> = ({ children, className = "" }) => (
  <motion.div className={`backdrop-blur-2xl bg-white/[0.03] border border-white/10 rounded-3xl shadow-2xl ${className}`}>
    {children}
  </motion.div>
);
...
export default function Home() {
  ...
  const onSubmitHandler = async (e: FormEvent) => {
    ...
    try {
      const { data } = await api.post<{ projectId: string }>(
        "/api/user/project",
        { initial_prompt: inputValue.trim() },
        { timeout: 30_000 }
      );
  ...
  return (
    <div className="min-h-screen bg-[#030303] text-white overflow-x-hidden selection:bg-purple-500/30 font-sans">
      <motion.div className="fixed inset-0 overflow-hidden pointer-events-none -z-10">
        <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-purple-600/10 blur-[130px] rounded-full" />
        <motion.div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-blue-600/10 blur-[130px] rounded-full" />
        <div
          ...
          <div className="max-w-7xl mx-auto mt-20">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
              {features.map((item, i) => (
                ...
                >
                  <motion.div className="w-12 h-12 rounded-xl bg-purple-500/10 flex items-center justify-center text-purple-400 mb-6">
                    {item.icon}
                  </motion.div>
